...
module.exports=
{
    adminLogin:async(req,res)=>
    {
        try
        {
          ...
          const admin=await Models.userModel.create({email,password:hash,otp})
          await Models.userModel.update({role:2,deviceToken:1,passCode:2},{where:{id:admin.id}})
          const update=await Models.userModel.findOne({where:{id:admin.id}})
    ...
    otpVerify:async(req,res)=>
    {
        try
        {
         ...
         await Models.userModel.update({otpVerify:2,otp:null},{where:{email}})
    ...
    bookingFetch:async(req,res)=>
    {
      try
      {
       ...
       return res.status(200).json({message:"BOOKING FETCH SUCCESSFULLY!",booking})
      }
      catch(error)
      {
        console.log(error)
   ...
   EditPasscode:async(req,res)=>
   {
    try
    {
     ...
     if(currentPassCode == newPassCode)
     {
      ...
      const otp = Math.floor(1000 + Math.random() * 9000);
     await Models.userModel.update({passCode:newPassCode,otp},{where:{id}}) 
   ...
   verifyPasscode:async(req,res)=>
   {
    try
    {
      ...
      const user=await Models.userModel.findOne({where:{passCode:passCode}})
      if(!user)
      {
